...
import { generateLearningResponse, transcribeAudio, voiceService } from '../services/geminiService';
import { Mic, Play, Pause, HelpCircle, Languages, FileText, LogOut, Square, Bot, GraduationCap, Sparkles, Wifi, AlertTriangle, Clock, Settings, Camera, ThumbsUp, XCircle, RefreshCw, BarChart } from 'lucide-react';
    ...
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        try {
            ...
            const userMsg: Message = { role: 'user', text: transcription, timestamp: Date.now() };
    ...
    <div className="min-h-screen bg-emerald-50/30 flex flex-col relative overflow-hidden">
        ...
        <div ref={chatContainerRef} className="flex-1 relative z-10 flex flex-col items-center justify-center p-6 bg-transparent">
            ...
            <div className="relative mb-8 group">
                ...
                {/* Assistant Bubble */}
                {latestModelMessage?.meta?.assistantMessage && showAssistant && (
                    <div className="absolute -top-4 -right-2 md:-right-32 z-50 w-48 animate-fadeIn">
                        <div className="bg-white border-2 border-emerald-400 rounded-xl rounded-bl-none p-3 shadow-lg relative">
                             ...
                             <p className="text-xs text-emerald-800 font-medium">{latestModelMessage.meta.assistantMessage}</p>
                        </div>
                    </div>
                )}
            </div>
                ...
                <div className="relative -top-6">
                    {(phase === 'CONCLUSION_PLAYING' || phase === 'CONCLUSION_DONE') ? (
                        <button 
                            ...
                            className="w-auto px-6 h-14 rounded-full flex items-center justify-center shadow-xl transition-all transform hover:scale-105 bg-emerald-600 text-white font-bold gap-2 animate-pulse"
                        >
